...
def is_process_running(process_name: str) -> bool:
    """Checks whether a named editor process is running on the current platform."""
    try:
        if platform.system().lower() == "windows":
            output = subprocess.check_output("tasklist", shell=True).decode("utf-8", errors="ignore")
            return process_name.lower() in output.lower()

        subprocess.check_output(["pgrep", "-x", process_name], stderr=subprocess.DEVNULL)
        return True
...
def main() -> None:
    ...
    if platform.system().lower() == "windows":
        old_process = "Antigravity.exe"
        new_process = "Antigravity IDE.exe"
    else:
        old_process = "Antigravity"
        new_process = "Antigravity IDE"

    is_old_running = is_process_running(old_process)
    is_new_running = is_process_running(new_process)
    ...
    try:
        ...
        logger.info("2. Starting root app state synchronization...")
        file_migrator.migrate_root_state()

        logger.info("3. Starting user data synchronization...")
        file_migrator.migrate_user_data()

        logger.info("4. Starting extensions (plugins) migration...")
        file_migrator.migrate_extensions()

        logger.info("5. Starting Gemini conversations and brain data synchronization...")
        file_migrator.migrate_gemini_data()

        logger.info("6. Starting SQLite database merging...")
        db_migrator.migrate()
